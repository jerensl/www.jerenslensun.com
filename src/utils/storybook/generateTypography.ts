...
export function createTypography(
    ...
    const {
        ...
        fontWeightBold = 500,
        // font size of HTML Element, 16px is the default font-size used by browsers.
        htmlFontSize = 14,
    } = typographyInput

    if (process.env.NODE_ENV !== 'production') {
        if (typeof fontSize !== 'number') {
            console.error('Storybook: `fontSize` is required to be a number.')
        }

        if (typeof htmlFontSize !== 'number') {
            console.error(
                'Storybook: `htmlFontSize` is required to be a number.'
            )
        }
    }
    ...
    const buildVariant = (
        ...
        if (lineHeight % 2 !== 0 || fontSize % 2 !== 0) {
            throw new Error(
                'Font size and line height should be integer multiples of 2 to prevent issues with alignment'
            )
        }
