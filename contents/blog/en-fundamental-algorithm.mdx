...
Imagine when we compare how fast a racer is at speed between different modes of transportation such as cars, motorbikes and so on, of course the definition of fast for each transportation will be different, even though they are both categorized as transportation. This is same as well as the algorithm used to run on various operating systems and different hardware will result in different speeds. The next question is how do we measure an algorithm with various variations of the system, if we measure it using real time speed, an algorithm will depend heavily on the system used.

In computer science, there are many ways to solve a problem with different algorithms. Therefore, we need to use methods to compare solutions in order to judge which one is more optimal. The method must meet several criteria:
...
1. Big-θ(Big-Theta) notation An algorithm can be said to be Big-θ or tight bound when it increases asymptotically exactly the same as the function of the running time that we use.  
   ...
   Description:
    ...
    - $T(n)$ represents the real time the code is run.
